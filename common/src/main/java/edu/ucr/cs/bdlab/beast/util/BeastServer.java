...
public class BeastServer extends AbstractHandler implements JCLIOperation {
  ...
  protected final Map<Class<? extends WebHandler>, WebHandler> webHandlers = new HashMap<>();

  static class WebMethodInfo {
    final Pattern pattern;
    final Class< ? extends WebHandler> webHandlerClass;
    final Method javaMethod;
    final WebMethod methodAnnotation;

    public WebMethodInfo(Pattern p, Class< ? extends WebHandler> w, Method m, WebMethod a) {
      this.pattern = p;
  ...
  protected WebMethodInfo[] loadWebMethods() {
    ...
    for (String handlerClassName : handlerClassNames) {
      try {
        Class<? extends WebHandler> handlerClass = Class.forName(handlerClassName).asSubclass(WebHandler.class);
  ...
  public void handle(String target, HttpServletRequest request, HttpServletResponse response, int d) {
    ...
    try {
      ...
      if (i < webMethods.length) {
        Class<? extends WebHandler> matchingWebHandler = webMethods[i].webHandlerClass;
        Method matchingMethod = webMethods[i].javaMethod;
        // Call the method
        WebHandler webHandler = webHandlers.get(matchingWebHandler);
        if (webHandler == null) {
      ...
      try {
        ...
        WebHandler.reportError(response, String.format("Error in request '%s' after %f seconds", target, (t2-t1)*1E-9), e);
