...
/**
 * The <code>GeoTiffMetadata2CRSAdapter</code> is responsible for interpreting the metadata provided
...
public final class GeoTiffMetadata2CRSAdapter {
    ...
    private static final Logger LOGGER =
            org.geotools.util.logging.Logging.getLogger(GeoTiffMetadata2CRSAdapter.class);
    ...
    /**
     * {@link Hints} to control the creation of the factories for this {@link
     * GeoTiffMetadata2CRSAdapter} object.
     */
    private Hints hints;
    ...
    /**
     * This method creates a <code>CoordinateReferenceSystem</code> object from the metadata which
     * has been set earlier. If it cannot create the <code>CoordinateReferenceSystem</code>, then
     * one of three exceptions is thrown to indicate the error.
     *
     * @param metadata  the metadata extracted from the GeoTIFF file
    ...
    private ProjectedCRS createProjectedCoordinateReferenceSystem(
            GeoTiffMetadata metadata) throws Exception {
        ...
        //
        // //
        String projCode = metadata.getGeoKey(ProjectedCSTypeGeoKey);
        if (projCode == null) {
        ...
        if (projCode.equalsIgnoreCase("unnamed")
                || projCode.equals(GTUserDefinedGeoKey_String)) {
        ...
        try {
            ...
            final ProjectedCRS pcrs =
                    (ProjectedCRS)
                            allAuthoritiesFactory.createCoordinateReferenceSystem(
                                    projCode);
            // //
            //
            ...
            boolean isDefaultUnit =
                    metadata.getGeoKey(ProjLinearUnitsGeoKey) == null
                            && linearUnit != null
    ...
    /**
     * Creation of a geographic coordinate reference system as specified in the GeoTiff
     * specification. User defined values are supported for all the possible levels of the above
    ...
    private GeographicCRS createGeographicCoordinateReferenceSystem(
            final GeoTiffMetadata metadata) throws IOException {
        GeographicCRS gcs = null;

        //
        // Get the crs code
        //
        final String tempCode = metadata.getGeoKey(GeographicTypeGeoKey);
        ...
        if (tempCode == null || tempCode.equals(GTUserDefinedGeoKey_String)) {
            //
     ...
     */
    private static int getGeoKeyAsInt(final int key, final GeoTiffMetadata metadata) {
     ...
     * ALWAYS the OGC {@link PixelInCell#CELL_CENTER} convention for the {@link ImageDatum} of the
     * underlying {@link ImageCRS}.
     *
     * @see <a href="http://lists.maptools.org/pipermail/geotiff/2006-January/000213.html">this
     *     email post</a>
     * @param metadata containing the information to build the {@link MathTransform} for going from
     *     grid to world.
     * @return the created MathTransform
     * @throws GeoTiffException if an error happens while creating the MathTransform
     */
    public static MathTransform getRasterToModel(final GeoTiffMetadata metadata)
        ...
        // decide raster type
        int rasterType = getGeoKeyAsInt(GTRasterTypeGeoKey, metadata);
        // geotiff spec says that PixelIsArea is the default
        if (rasterType == UNDEFINED) {
        ...
        if (hasTiePoints && hasPixelScales) {
            ...
            final double tiePointColumn =
                    tiePoints[0].getValueAt(0)
                            + (rasterType == RasterPixelIsArea ? -0.5 : 0);
            ...
            if (rasterType == RasterPixelIsArea) {
                ...
                assert rasterType == RasterPixelIsPoint;
        ...
        try {
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.log(
                        Level.FINE,
                        "Invalid model transformation found!\n" + e.getLocalizedMessage(),
                        e);
            }
     ...
     */
    public static PixelInCell getRasterType(GeoTiffMetadata metadata) {
        int rasterType = getGeoKeyAsInt(GTRasterTypeGeoKey, metadata);
        ...
        } else if (rasterType == RasterPixelIsPoint) {
    ...
    private static double getGeoKeyAsDouble(
            final int key, final GeoTiffMetadata metadata) {
    ...
    private ProjectedCRS createUserDefinedPCS(
            final GeoTiffMetadata metadata, Unit<?> linearUnit) throws Exception {
        ...
        //
        String pcsCitationGeoKey = metadata.getGeoKey(PCSCitationGeoKey);
        ...
        // ProjLinearUnitsGeoKey
        //
        final String projCode = metadata.getGeoKey(ProjectionGeoKey);
        boolean projUserDefined = false;
        if (projCode == null || projCode.equals(GTUserDefinedGeoKey_String)) {
        ...
        if (projUserDefined) {
            String citationName = metadata.getGeoKey(GTCitationGeoKey);
    ...
    /**
     * Clean the provided parameters <code>tiffName</code> from strange strings like it happens with
     * erdas imagine.
     *
    ...
    private PrimeMeridian createPrimeMeridian(
            final GeoTiffMetadata metadata, Unit linearUnit) throws IOException {
        // look up the prime meridian:
        // + could be an EPSG code
        // + could be user defined
        // + not defined = greenwich
        final String pmCode = metadata.getGeoKey(GeogPrimeMeridianGeoKey);
        PrimeMeridian pm = null;

        try {
            if (pmCode != null) {
                if (pmCode.equals(GTUserDefinedGeoKey_String)) {
                    try {
                        String name = metadata.getGeoKey(GeogCitationGeoKey);
                        if (name == null) {
                            name = "unnamed";
                        } else {
                        ...
                        final String pmValue =
                                metadata.getGeoKey(GeogPrimeMeridianLongGeoKey);
    ...
    /**
     * Looks up the Geodetic Datum as specified in the GeoTIFFWritingUtilities file. The geotools
     * definition of the geodetic datum includes both an ellipsoid and a prime meridian, but the
     * code in the GeoTIFFWritingUtilities file does NOT include the prime meridian, as it is
     * specified separately. This code currently does not support user defined datum.
     *
    ...
    private GeodeticDatum createGeodeticDatum(
            final Unit<?> unit, final GeoTiffMetadata metadata) throws IOException {
        // lookup the datum (w/o PrimeMeridian), error if "user defined"
        GeodeticDatum datum = null;
        final String datumCode = metadata.getGeoKey(GeogGeodeticDatumGeoKey);
        ...
        if (datumCode.equals(GTUserDefinedGeoKey_String)) {
            /**
             * USER DEFINED DATUM
             *
             * <p>From the spec:
             *
             * <p>GeoKey Requirements for User-Defined Horizontal Datum: GeogCitationGeoKey
             * GeogEllipsoidGeoKey
             */
            // datum name
            String name = metadata.getGeoKey(GeogCitationGeoKey);
            ...
            try {
                datum =
                        (GeodeticDatum)
                                (this.allAuthoritiesFactory.createDatum("EPSG:" + datumCode));
            } catch (Exception cce) {
                final GeoTiffException ex =
                        new GeoTiffException(metadata, cce.getLocalizedMessage(), cce);
                throw ex;
            }
        }
     ...
     */
    private Ellipsoid createEllipsoid(final Unit unit, final GeoTiffMetadata metadata)
            throws IOException {
        // /////////////////////////////////////////////////////////////////////
        //
        ...
        // /////////////////////////////////////////////////////////////////////
        // ellipsoid key
        final String ellipsoidKey = metadata.getGeoKey(GeogEllipsoidGeoKey);
        String temp = null;
        // is the ellipsoid user defined?
        if (ellipsoidKey.equals(GTUserDefinedGeoKey_String)) {
            //
            ...
            temp = metadata.getGeoKey(GeogSemiMajorAxisGeoKey);
            final double semiMajorAxis = (temp != null ? Double.parseDouble(temp) : Double.NaN);
            temp = metadata.getGeoKey(GeogInvFlatteningGeoKey);
            final double inverseFlattening;
            if (temp != null) {
                inverseFlattening = (temp != null ? Double.parseDouble(temp) : Double.NaN);
            } else {
                temp = metadata.getGeoKey(GeogSemiMinorAxisGeoKey);
    ...
    private GeographicCRS createUserDefinedGCS(
            final GeoTiffMetadata metadata, Unit<?> linearUnit, Unit<?> angularUnit)
            throws IOException {
        // //
        //
    ...
    private ParameterValueGroup createUserDefinedProjectionParameter(
            String name, final GeoTiffMetadata metadata)
        ...
        //
        // ///
        final String coordTrans = metadata.getGeoKey(ProjCoordTransGeoKey);

        // throw descriptive exception if ProjCoordTransGeoKey not defined
        if ((coordTrans == null)
                || coordTrans.equalsIgnoreCase(GTUserDefinedGeoKey_String)) {
            throw new GeoTiffException(
                    metadata,
    ...
    /**
     * Set the projection parameters basing its decision on the projection name. I found a complete
     * list of projections on the geotiff website at address
     * http://www.remotesensing.org/geotiff/proj_list.
     *
     * <p>I had no time to implement support for all of them therefore you will not find all of
     * them. If you want go ahead and add support for the missing ones. I have tested this code
     * against some geotiff files you can find on the geotiff website under the ftp sample directory
     * but I can say that they are a real mess! I am respecting the specification strictly while
     * many of those fields do not! I could make this method trickier and use workarounds in order
    ...
    private ParameterValueGroup setParametersForProjection(
            String name, final String coordTransCode, final GeoTiffMetadata metadata)
            throws GeoTiffException {
        ...
        try {
            ...
            if (name.equalsIgnoreCase("transverse_mercator")
                ...
                parameters
                        ...
                        .setValue(
                                getGeoKeyAsDouble(
                                        ProjScaleAtNatOriginGeoKey, metadata));
            ...
            if (name.equalsIgnoreCase("mercator_1SP")
                ...
                final double standard_parallel_1 =
                        getGeoKeyAsDouble(ProjStdParallel1GeoKey, metadata);
            ...
            if (name.equalsIgnoreCase("lambert_conformal_conic_2SP")
                ...
                parameters
                        ...
                        .setValue(
                                getGeoKeyAsDouble(
                                        ProjStdParallel1GeoKey, metadata));
                parameters
                        .parameter("standard_parallel_2")
                        .setValue(
                                getGeoKeyAsDouble(
                                        ProjStdParallel2GeoKey, metadata));
                parameters.parameter("false_easting").setValue(getFalseEasting(metadata));
                parameters.parameter("false_northing").setValue(getFalseNorthing(metadata));
            ...
            // .setValue(
            // this
            // .getGeoKeyAsDouble(GeoTiffMetadata.ProjStdParallel1GeoKey));
            // parameters
            // .parameter("standard_parallel_2")
            // .setValue(
            // this
            // .getGeoKeyAsDouble(GeoTiffMetadata.ProjStdParallel2GeoKey));
            ...
            if (name.equalsIgnoreCase("oblique_mercator")
                ...
                parameters
                        ...
                        .setValue(
                                getGeoKeyAsDouble(
                                        ProjAzimuthAngleGeoKey, metadata));
                parameters.parameter("false_easting").setValue(getFalseEasting(metadata));
                parameters.parameter("false_northing").setValue(getFalseNorthing(metadata));
                parameters.parameter("longitude_of_center").setValue(getOriginLong(metadata));
                parameters.parameter("latitude_of_center").setValue(getOriginLat(metadata));
        ...
        } catch (NoSuchIdentifierException | IOException e) {
     ...
     */
    private static double getScaleFactor(final GeoTiffMetadata metadata) throws IOException {
        String scale = metadata.getGeoKey(ProjScaleAtCenterGeoKey);
        if (scale == null) scale = metadata.getGeoKey(ProjScaleAtNatOriginGeoKey);
     ...
     */
    private static double getFalseEasting(final GeoTiffMetadata metadata) throws IOException {
        String easting = metadata.getGeoKey(ProjFalseEastingGeoKey);
        if (easting == null)
            easting = metadata.getGeoKey(ProjFalseOriginEastingGeoKey);
        if (easting == null) return 0.0;
        return Double.parseDouble(easting);
    }

    /**
     * Getting the false northing with a minimum of tolerance with respect to the parameters name. I
     * saw that often people use the wrong geokey to store the false easting, we cannot be too picky
     * we need to get going pretty smoothly.
     *
     ...
     */
    private static double getFalseNorthing(final GeoTiffMetadata metadata) throws IOException {
        String northing = metadata.getGeoKey(ProjFalseNorthingGeoKey);
        if (northing == null)
            northing = metadata.getGeoKey(ProjFalseOriginNorthingGeoKey);
     ...
     */
    private static double getOriginLong(final GeoTiffMetadata metadata) throws IOException {
        String origin = metadata.getGeoKey(ProjCenterLongGeoKey);
        if (origin == null) origin = metadata.getGeoKey(ProjNatOriginLongGeoKey);
        if (origin == null) origin = metadata.getGeoKey(ProjFalseOriginLongGeoKey);
        if (origin == null)
            origin = metadata.getGeoKey(ProjStraightVertPoleLongGeoKey);
        if (origin == null) origin = metadata.getGeoKey(ProjFalseNorthingGeoKey);
     ...
     */
    private static double getOriginLat(final GeoTiffMetadata metadata) throws IOException {
        String origin = metadata.getGeoKey(ProjCenterLatGeoKey);
        if (origin == null) origin = metadata.getGeoKey(ProjNatOriginLatGeoKey);
        if (origin == null) origin = metadata.getGeoKey(ProjFalseOriginLatGeoKey);
        if (origin == null) return 0.0;
    ...
    private <Q extends Quantity<Q>> Unit<Q> createUnit(
            ...
            Unit<Q> base,
            Unit<Q> def,
            final GeoTiffMetadata metadata)
            throws IOException {
        ...
        //
        // //
        if (unitCode.equals(GTUserDefinedGeoKey_String)) {
            try {
