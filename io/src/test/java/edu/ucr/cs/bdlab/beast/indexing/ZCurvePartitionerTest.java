...
public class ZCurvePartitionerTest extends JavaSparkTest {

  public void testPartition2DPoints() throws IOException {
    double[][] coords = readCoordsResource("/test.points");
    ZCurvePartitioner partitioner = new ZCurvePartitioner();
    ...
    assertEquals(12.0, partitioner.splitPoints[0][1]);
    assertEquals(2.0, partitioner.splitPoints[1][1]);
    ...
    env.set(new double[] {4.0, 9.0}, new double[]{4.0, 9.0});
    assertEquals(1, partitioner.overlapPartition(env));
    env.set(new double[] {9.0, 8.0}, new double[]{9.0, 8.0});
