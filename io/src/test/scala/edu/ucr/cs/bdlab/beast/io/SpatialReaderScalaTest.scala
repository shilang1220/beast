...
class SpatialReaderScalaTest extends FunSuite with ScalaSparkTest {
  ...
  test("ReadIndexedFile") {
    ...
    assert(loadedFile.partitioner.get.isInstanceOf[SparkSpatialPartitioner])
    val mbr0 = new EnvelopeNDLite()
    loadedFile.partitioner.get.asInstanceOf[SparkSpatialPartitioner]
      .getSpatialPartitioner.asInstanceOf[CellPartitioner].getPartitionMBR(0, mbr0)
