...
class SpatialFileRDDTest extends FunSuite with ScalaSparkTest {
  ...
  test("Skip duplicate avoidance") {
    ...
    val features = new SpatialFileRDD(sparkContext, indexPath.getPath,
      Seq(SpatialFileRDD.InputFormat -> "envelopek(2)", CSVFeatureReader.FieldSeparator -> ',',
        SpatialFileRDD.DuplicateAvoidance -> false))
  ...
  test("recursive with shapefile") {
    val filePath = new File(scratchDir, "dir")
    val shpPath = new File(filePath, "subdir/deeper/tree/")
    copyDirectoryFromResources("/usa-major-cities", shpPath)
    ...
    assert(features.partitions(0).asInstanceOf[SpatialFilePartition].path.endsWith("usa-major-cities.shp"))
  }
  ...
  test("Detect the input") {
    ...
    inputPath = makeDirCopy("/usa-major-cities")
    features = new SpatialFileRDD(sparkContext, inputPath.getPath)
    assert(features.featureReaderClass == classOf[ShapefileFeatureReader])
  }
  ...
  test("Should create a partitioned dataset from a master file") {
    ...
    assert(features.partitioner.get.isInstanceOf[SparkSpatialPartitioner])
    assert(features.partitioner.get.asInstanceOf[SparkSpatialPartitioner].getSpatialPartitioner.getPartitionCount == 2)
  }
  ...
  test("local read") {
    ...
    for (partition <- partitions) {
      val features = SpatialFileRDD.readPartition(partition, featureReaderClass, opts)
