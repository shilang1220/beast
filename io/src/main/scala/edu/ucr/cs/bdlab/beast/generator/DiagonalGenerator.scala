...
import edu.ucr.cs.bdlab.beast.generator.DiagonalGenerator.{Buffer, Percentage}
...
class DiagonalGenerator(partition: RandomSpatialPartition)
  ...
  val percentage: Double = partition.opts.getDouble(Percentage, 0.5)

  val buffer: Double = partition.opts.getDouble(Buffer, 0.2)
