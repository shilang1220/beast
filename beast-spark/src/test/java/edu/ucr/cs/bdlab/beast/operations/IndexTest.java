...
import edu.ucr.cs.bdlab.beast.indexing.IndexRecordWriter;
...
public class IndexTest extends JavaSparkTest {
   ...
   * (tab-separated values) with a header line {@link edu.ucr.cs.bdlab.beast.indexing.IndexRecordWriter#CSVHeader} and one line per partition. This method is supposed
  ...
  public static IFeature[] readPartitions(FileSystem fs, Path file) throws IOException {
    ...
    try {
      ...
      int numDimensions = (line.toString().split("\t").length - IndexRecordWriter.CSVHeader.split("\t").length) / 2;
      reader.close();
      reader = null;
      ...
      Iterator<IFeature> features = SpatialFileRDD.readLocalJ(file.toString(), iformat,
          new BeastOptions().setBoolean(CSVFeatureReader.SkipHeader, true), fs.getConf());
  ...
  public void testBuildIndex() throws IOException {
    ...
    opts.setBoolean(IndexHelper.BalancedPartitioning(), false);
  ...
  public void testOverwriteOutput() throws IOException {
    ...
    BeastOptions opts = new BeastOptions(false);
    opts.setBoolean(SpatialOutputFormat.OverwriteOutput, true);
    opts.set(SpatialFileRDD.InputFormat(), "pointk(4)");
    opts.set(CSVFeatureReader.FieldSeparator, ",");
    opts.set(IndexHelper.PartitionCriterionThreshold(), "Fixed(4)");
    opts.set(IndexHelper.GlobalIndex(), "rsgrove");
  ...
  public void testCreatePartitioner() {
    ...
    BeastOptions opts = new BeastOptions(false);
    opts.setInt(IndexHelper.SynopsisSize(), 100);
  ...
  public void testPartitionFeatures() {
    ...
    clo.setInt(IndexHelper.SynopsisSize(), 10);
    ...
    JavaPairRDD<Integer, IFeature> partitionedData = IndexHelper.partitionFeatures(featuresRDD, partitioner);
  ...
  public void testPartitionWithReplication() {
    ...
    JavaRDD<IFeature> featuresRDD = javaSparkContext().parallelize(features);
    JavaPairRDD<Integer, IFeature> partitionedFeatures = IndexHelper.partitionFeatures(featuresRDD, p);
  ...
  public void testWriteIndex() throws IOException {
    ...
    BeastOptions opts = new BeastOptions(false);
    opts.set(SpatialOutputFormat.OutputFormat, "point(0,1)");
    clo.set("separator", "\t");
    IndexHelper.saveIndex(partitionedData, indexPath.toString(), opts);
  ...
  public void testWritePartitionedIndex() throws IOException {
    ...
    BeastOptions clo = new BeastOptions(false).setInt(IndexHelper.SynopsisSize(), 10);
    GridPartitioner partitioner = (GridPartitioner) IndexHelper.createPartitioner(featuresRDD, GridPartitioner.class,
        "fixed", 4, null, new BeastOptions(false));
  ...
  public void testWriteIndexWithFourDimensions() throws IOException {
    ...
    BeastOptions opts = new BeastOptions(false);
    opts.set(SpatialFileRDD.InputFormat(), "pointk(4)");
    opts.set(SpatialOutputFormat.OutputFormat, "pointk(4)");
    opts.set(CSVFeatureReader.FieldSeparator, ",");
    ...
    SpatialPartitioner partitioner = IndexHelper.createPartitioner(featuresRDD, GridPartitioner.class,
        "fixed", 16, null, new BeastOptions(false).setInt(IndexHelper.SynopsisSize(), 1000));
    JavaPairRDD<Integer, IFeature> partitionedData = IndexHelper.partitionFeatures(featuresRDD, partitioner);
    Path indexPath = new Path(scratchPath(), "temp_index");
  ...
  public void testWriteRSGroveIndexWithFourDimensions() throws IOException {
    ...
    SpatialPartitioner partitioner = IndexHelper.createPartitioner(featuresRDD, RSGrovePartitioner.class,
        "fixed", 16, null, new BeastOptions(false)
            .setInt(IndexHelper.SynopsisSize(), 1000)
            .setBoolean(IndexHelper.BalancedPartitioning(), false));
    JavaPairRDD<Integer, IFeature> partitionedData = IndexHelper.partitionFeatures(featuresRDD, partitioner);
    Path indexPath = new Path(scratchPath(), "temp_index");
    // Write the index to the output
    IndexHelper.saveIndex(partitionedData, indexPath.toString(), opts);
  ...
  public void testEndToEndIndexing() throws IOException {
    ...
    for (String partitioner : allPartitioners) {
      ...
      clo.set(SpatialOutputFormat.OutputFormat, "pointk(2)");
  ...
  public void testEndToEndWithAllIndexersWithTenPartition() throws IOException {
    ...
    for (String partitioner : allPartitioners) {
      try {
        ...
        opts.set(SpatialOutputFormat.OutputFormat, "pointk(2)");
        opts.set(CSVFeatureReader.FieldSeparator, ",");
