...
public class SimpleGraphics extends Graphics2D {
  ...
  /**
   * Sets the pixel at the given coordinates. It also applies the transformation.
  ...
  /**
   * Writes the same color to all pixels in the given block without applying the transformation to the coordinates.
   * @param x lower coordinate of the rectangle in pixels
   * @param y lower coordinate of the rectangle in pixels
   * @param width the width of the rectangle in pixels. A width of 1 means one pixel
   * @param height the height of the rectangle in pixels. A height of 1 means one pixel
  ...
  @Override
  public void drawOval(int x, int y, final int width, final int height) {
    assert width == height : "Only circle are supported for now";
    x += tx; y += ty;
    int xc = x + width / 2;
    int yc = y + height / 2;
    int r = width / 2;
  ...
  private void drawCircleParts(int xc, int yc, int dx, int dy) {
    setPixel(xc+dx, yc+dy, color.getRGB());
    setPixel(xc-dx, yc+dy, color.getRGB());
    setPixel(xc+dx, yc-dy, color.getRGB());
    setPixel(xc-dx, yc-dy, color.getRGB());
    setPixel(xc+dy, yc+dx, color.getRGB());
    setPixel(xc-dy, yc+dx, color.getRGB());
    setPixel(xc+dy, yc-dx, color.getRGB());
    setPixel(xc-dy, yc-dx, color.getRGB());
  }
  ...
  private void fillCircleParts(int xc, int yc, int dx, int dy) {
    dumpRect(xc-dx, yc+dy, 2 * dx, 1, color);
    dumpRect(xc-dx, yc-dy, 2 * dx, 1, color);
    dumpRect(xc-dy, yc+dx, 2 * dy, 1, color);
    dumpRect(xc-dy, yc-dx, 2 * dy, 1, color);
  }
