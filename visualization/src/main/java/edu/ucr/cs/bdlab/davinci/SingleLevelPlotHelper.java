  ...
  public static void plotLocal(Path inPath, DataOutputStream output,
    ...
    if (mercator) {
      ...
      Envelope convertedMBR = Reprojector.reprojectEnvelope(mbr,
          FeatureReader.DefaultCRS, CommonVisualizationHelper.MercatorCRS, CRSServer.sparkConfFromHadoopConf(conf));
    ...
    SpatialFileRDD.SpatialFilePartition[] partitions = SpatialFileRDD.createPartitions(inPath.toString(), opts, conf);
    ...
    try {
      List<Canvas> partialCanvases = Parallel.forEach(partitions.length, (i1, i2) -> {
        try {
              ...
              if (mercator) {
                ...
                if (transformationInfo == null) {
                  ...
                  transformationInfo = Reprojector.findTransformationInfo(feature.getGeometry().getSRID(),
                      CommonVisualizationHelper.MercatorCRS, CRSServer.sparkConfFromHadoopConf(conf));
                }
          ...
          throw new RuntimeException(String.format("Error reprojecting geometry'"), e);
        }
      });
      Plotter plotter = plotterClass.newInstance();
      plotter.setup(new BeastOptions(conf));
