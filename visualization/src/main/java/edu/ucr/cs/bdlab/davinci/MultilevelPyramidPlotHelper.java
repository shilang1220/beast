...
public class MultilevelPyramidPlotHelper {
   ...
   * @return whether the tile was non-empt, {@code true} if the produced image is not empty
   * @throws IOException if an error happens while reading the input
   */
  public static boolean plotTile(FileSystem fs, Path vizPath, long tileID, OutputStream out, LongWritable clientTimestamp)
    ...
    BeastOptions opts = fs.exists(vizMetadataPath)?
        new BeastOptions().loadFromTextFile(fs, vizMetadataPath)
        : new BeastOptions(fs.getConf());

    String imageExtension = Plotter.getImageExtension(opts.getString(CommonVisualizationHelper.PlotterName));
    String dataExtension = FeatureReader.getFileExtension(opts.getString(SpatialFileRDD.InputFormat()));
    do {
    ...
    if (opts.getBoolean(CommonVisualizationHelper.UseMercatorProjection, false)) {
      ...
      inputMBR = new EnvelopeNDLite(Reprojector.reprojectEnvelope(
          ...
          FeatureReader.DefaultCRS, CommonVisualizationHelper.MercatorCRS, CRSServer.sparkConfFromHadoopConf(fs.getConf()))) ;
      // Always apply vflip
      t.vflip();
    } else {
    ...
    if (opts.getBoolean(CommonVisualizationHelper.UseMercatorProjection, false)) {
      ...
      tileMBR = Reprojector.reprojectEnvelope(tileMBR, CommonVisualizationHelper.MercatorCRS, FeatureReader.DefaultCRS,
          CRSServer.sparkConfFromHadoopConf(fs.getConf()));
    }
    opts.set("mbr", CSVEnvelopeEncoder.defaultEncoderJTS.apply(tileMBR));
  ...
  public static void writeVisualizationAddOnFiles(FileSystem fs, Path vizPath, BeastOptions opts) throws IOException {
    ...
    Option<String> oformat = opts.get(SpatialOutputFormat.OutputFormat);
