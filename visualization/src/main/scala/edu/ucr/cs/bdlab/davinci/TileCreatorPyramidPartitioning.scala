...
class TileCreatorPyramidPartitioning(sortedFeatures: Iterator[(Long, IFeature)], partitioner: PyramidPartitioner,
  ...
  def generateNextBatch(): Iterator[(Long, Canvas)] = {
    ...
    do {
      ...
      //System.err.println(s"@${System.currentTimeMillis()}: Started generating tiles for ${pyramid}")
      do {
        recordCount += 1
      ...
      //System.err.println(s"Generated ${tempTiles.size} tiles from ${recordCount} records in pyramid ${pyramid} in ${(t2-t1)*1E-9} seconds")
  ...
  override def next(): (Long, Canvas) = {
    val returnTuple: (Long, Canvas) = iGeneratedTiles.next()
