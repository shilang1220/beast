...
public class MultilevelPyramidPlotHelperTest extends JavaSparkTest {
  ...
  public void testClassifyTile() {
    ...
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ShallowTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 2, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.DataTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 1, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ImageTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 0, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.EmptyTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 2, 1, 0));
  }
  ...
  public void testClassifyTileWithPartialHistogram() {
    ...
    Prefix2DHistogram h = new Prefix2DHistogram(histogram);
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ShallowTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 2, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ShallowTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 2, 1, 1));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.DataTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 1, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ImageTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 0, 0, 0));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ShallowTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 2, 2, 3));

    histogram = new UniformHistogram(inputMBR, 4096, 4096);
    histogram.addEntry(new int[] {0, 0}, 100);
    h = new Prefix2DHistogram(histogram);
    assertEquals(MultilevelPyramidPlotHelper.TileClass.EmptyTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 14, 4201, 6086));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.ShallowTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 14, 3, 3));
    assertEquals(MultilevelPyramidPlotHelper.TileClass.EmptyTile, MultilevelPyramidPlotHelper.classifyTile(h, 150, 14, 4, 4));
  }
  ...
  public void testPlotTile() throws IOException {
    ...
    copyDirectoryFromResources("/pyramid", new File(pyramidPath.toString()));
  ...
  public void testPlotTilePointSize() throws IOException, InterruptedException {
    ...
    PrintStream datafile = new PrintStream(fs.create(dataPath));
    try {
  ...
  public void testPlotTileGeoJSON() throws IOException, InterruptedException {
    Path pyramidPath = new Path(scratchPath(), "pyramid");
    copyDirectoryFromResources("/pyramid_geojson", new File(pyramidPath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test the root image tile
  ...
  public void testPlotTileSkipIfCached() throws IOException, InterruptedException {
    ...
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(0, 0, 0), baos, tileTimestamp));
    assertEquals(UnitDelay * 2, tileTimestamp.get() - baseTimestamp);
    baos.close();
    // Requesting it again should retrieve the cached image
    baos = new ByteArrayOutputStream();
    assertFalse(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(0, 0, 0), baos, tileTimestamp));
    baos.close();

    // Test a data tile with a non-cached version
    baos = new ByteArrayOutputStream();
    tileTimestamp = new LongWritable(0);
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(1, 1, 0), baos, tileTimestamp));
    assertEquals(UnitDelay * 5, tileTimestamp.get() - baseTimestamp);
    baos.close();

    // Requesting it again should not retrieve a new image
    baos = new ByteArrayOutputStream();
    assertFalse(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(1, 1, 0), baos, tileTimestamp));
    baos.close();
  ...
  public void testPlotTileRTree() throws IOException {
    ...
    copyDirectoryFromResources("/test_partial_pyramid", new File(pyramidPath.toString()));
    Path rtreePath = new Path(scratchPath(), "test_rtree");
    copyDirectoryFromResources("/test_rtree", new File(rtreePath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test the root image tile
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
  ...
  public void testPlotTileRTreeShouldSkipCachedTiles() throws IOException {
    ...
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    timestamp.set(0);
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(2, 0, 2), baos, timestamp));
    baos.close();
    assertEquals(baseTimestamp, timestamp.get());
    // Requesting a data tile again should not regenerate it
    baos = new ByteArrayOutputStream();
    assertFalse(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(2, 0, 2), baos, timestamp));
    baos.close();
  }
  ...
  public void testPlotTileMercator() throws IOException, InterruptedException {
    ...
    copyDirectoryFromResources("/test_pyramid_mercator", new File(pyramidPath.toString()));
    Path rtreePath = new Path(scratchPath(), "test_rtree");
    copyDirectoryFromResources("/test_rtree", new File(rtreePath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test the root image tile
  ...
  public void testPlotTileMercatorRTree() throws IOException {
    ...
    copyDirectoryFromResources("/rect_mercator_shallow_plot", new File(pyramidPath.toString()));
    Path rtreePath = new Path(scratchPath(), "rect_mercator_rgrove");
    copyDirectoryFromResources("/rect_mercator_rgrove", new File(rtreePath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
  ...
  public void testPlotTileMercatorHybrid() throws IOException {
    ...
    copyDirectoryFromResources("/rect_mercator_hybrid_plot", new File(pyramidPath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test a tile at level 1
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(1, 0, 0), baos));
  ...
  public void testPlotTileVFlip() throws IOException {
    Path pyramidPath = new Path(scratchPath(), "pyramid");
    Path dataPath = new Path(scratchPath(), "rect_mercator.rect");
    copyDirectoryFromResources("/test_mercator_vflip_shallow", new File(pyramidPath.toString()));
    copyResource("/rect_mercator.rect", new File(dataPath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test the root tile
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(0, 0, 0), baos));
  ...
  public void testPlotDataTileVFlip() throws IOException, InterruptedException {
    Path pyramidPath = new Path(scratchPath(), "pyramid");
    copyDirectoryFromResources("/test2_vflip_shallow", new File(pyramidPath.toString()));
    FileSystem fs = pyramidPath.getFileSystem(new Configuration());
    // Test the root tile
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    assertTrue(MultilevelPyramidPlotHelper.plotTile(fs, pyramidPath, TileIndex.encode(0, 0, 0), baos));
